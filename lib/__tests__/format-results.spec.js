...
test('can sort summary by rule id', () => {
  ...
  expect(stripAnsi(format(results, { EFS_SORT_BY: 'rule' }))).toMatchSnapshot();
  expect(
    stripAnsi(format(results, { EFS_SORT_BY: 'rule', EFS_SORT_DESC: 'true' }))
  ).toMatchSnapshot();
});
...
test('can sort summary by num of errors', () => {
  ...
  expect(stripAnsi(format(results, { EFS_SORT_BY: 'errors' }))).toMatchSnapshot();
  expect(
    stripAnsi(format(results, { EFS_SORT_BY: 'errors', EFS_SORT_DESC: 'true' }))
  ).toMatchSnapshot();
});
...
test('can sort summary by num of warnings', () => {
  ...
  expect(stripAnsi(format(results, { EFS_SORT_BY: 'warnings' }))).toMatchSnapshot();
  expect(
    stripAnsi(format(results, { EFS_SORT_BY: 'warnings', EFS_SORT_DESC: 'true' }))
  ).toMatchSnapshot();
});
