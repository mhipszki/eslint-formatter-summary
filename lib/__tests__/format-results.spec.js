...
import mockResult from './mock-result-factory.js';
...
test('can sort summary by rule id', () => {
  ...
  expect(stripAnsi(format(results, { sortByProp: 'rule' }))).toMatchSnapshot();
  expect(
    stripAnsi(format(results, { sortByProp: 'rule', sortDescending: 'true' }))
  ).toMatchSnapshot();
});
...
test('can sort summary by num of errors', () => {
  ...
  expect(
    stripAnsi(format(results, { sortByProp: 'errors' }))
  ).toMatchSnapshot();
  expect(
    stripAnsi(format(results, { sortByProp: 'errors', sortDescending: 'true' }))
  ).toMatchSnapshot();
});
...
test('can sort summary by num of warnings', () => {
  ...
  expect(
    stripAnsi(format(results, { sortByProp: 'warnings' }))
  ).toMatchSnapshot();
  expect(
    stripAnsi(
      format(results, { sortByProp: 'warnings', sortDescending: 'true' })
    )
  ).toMatchSnapshot();
});
