...
const constructHeader = rules => {
  const len = `errors ${'0'.repeat(maxErrorLen(rules))} warnings ${'0'.repeat(maxWarningLen(rules))} rule: ${'0'.repeat(maxRuleLen(rules))}`.length;
  const header = ` ${sparkles}  Summary of failing ESLint rules `.padEnd(len);
  return chalk`{bgBlue ${header}}`;
};

const constructSummary = rules => rules.map(rule => {
