 ...
 * @property {string|undefined} [EFS_SORT_BY]
 * @property {'true'|undefined} [EFS_SORT_DESC]
 */
 ...
 */
export default function format(results, options) {
  const { EFS_SORT_BY, EFS_SORT_DESC } = options;
  const rules = aggregate(results);

  if (EFS_SORT_BY && ['rule', 'errors', 'warnings'].includes(EFS_SORT_BY)) {
    const prop = EFS_SORT_BY === 'rule' ? 'ruleId' : EFS_SORT_BY;
    const direction = EFS_SORT_DESC === 'true' ? 'desc' : 'asc';
