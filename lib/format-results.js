 ...
 * @returns {string}
 */
const constructSummary = (rules, { markdown }) => {
  ...
  return rules
    .map((rule, i) => {
      ...
      const line = markdown
        ? `| ${errors} | ${warnings} | ${ruleId.padEnd(maxRule)} |`
...
export function format(results, options) {
  const { output, sortByProp, sortDescending } = options;
  ...
  const summary = constructSummary(rules, { markdown });
