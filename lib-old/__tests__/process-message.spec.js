...
test('adds new rules to summary', () => {
  const summary = [];
  expect(
    processMessage(summary, { ruleId: 'new rule', severity: 1 })
  ).toContainEqual({
    ruleId: 'new rule',
    errors: 0,
    warnings: 1,
...
test('skips rules with no ruleId', () => {
  const summary = [];
  expect(processMessage(summary, { ruleId: undefined, severity: 1 })).toEqual(
    []
  );
});
...
test('updates errors / warnings of existing rules', () => {
  ...
  expect(
    processMessage(summary, { ruleId: 'rule', severity: 1 })
  ).toContainEqual({
    ruleId: 'rule',
    errors: 0,
    warnings: 2,
